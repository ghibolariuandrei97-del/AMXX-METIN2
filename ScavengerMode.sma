...
new const g_szXPModel[] = "models/w_longjump.mdl";
new const g_szYangModel[] = "models/w_backpack.mdl";
...
new g_pXPMin, g_pXPMax, g_pYangMin, g_pYangMax;
...
public plugin_init() {
    ...
    g_pXPMin = register_cvar("amx_loot_xp_min", "50");
    g_pXPMax = register_cvar("amx_loot_xp_max", "200");
    g_pYangMin = register_cvar("amx_loot_yang_min", "500");
    g_pYangMax = register_cvar("amx_loot_yang_max", "2000");
...
public plugin_precache() {
    ...
    precache_model(g_szXPModel);
    precache_model(g_szYangModel);
    precache_model("models/w_c4.mdl");
    for(new i = 0; i < sizeof(g_szWeaponModels); i++) precache_model(g_szWeaponModels[i]);

    // Sunete folosite in OnLootTouch (Rezolva erorile SV_BuildSoundMsg)
...
public GenerateMapLoot() {
    ...
    for (new i = 0; i < iTotalItems; i++) {
        ...
        if (iRand <= 60) SpawnLootItem(random_num(0, charsmax(g_szWeaponNames)));
        else if (iRand <= 75) SpawnCustomLoot(100); // Bani
        else if (iRand <= 85) SpawnCustomLoot(200); // HP
        else if (iRand <= 93) SpawnCustomLoot(400); // XP Metin2
        else SpawnCustomLoot(500); // Yang Metin2
    }
}
...
public OnLootTouch(iEnt, iId) {
    ...
    if (iVal == 400) { // XP Metin2
        new iMin = get_pcvar_num(g_pXPMin);
        new iMax = get_pcvar_num(g_pXPMax);
        new iAmount = random_num(iMin, iMax);

        m2_add_xp(iId, iAmount);
        client_print(iId, print_center, "Ai gasit un Pergament: +%d XP!", iAmount);
        emit_sound(iId, CHAN_ITEM, "items/gunpickup2.wav", 1.0, ATTN_NORM, 0, PITCH_NORM);
        rg_remove_entity(iEnt);
        return;
    }

    if (iVal == 500) { // Yang Metin2
        new iMin = get_pcvar_num(g_pYangMin);
        new iMax = get_pcvar_num(g_pYangMax);
        new iAmount = random_num(iMin, iMax);

        m2_add_yang(iId, iAmount);
        client_print(iId, print_center, "Ai gasit o Punga: +%d Yang!", iAmount);
        emit_sound(iId, CHAN_ITEM, "items/gunpickup2.wav", 1.0, ATTN_NORM, 0, PITCH_NORM);
        rg_remove_entity(iEnt);
        return;
    }
...
stock SpawnCustomLoot(iType) {
    ...
    if (FindSurface(fOrigin)) {
        ...
        if (is_valid_ent(iEnt)) {
            ...
            new szModel[64];
            switch (iType) {
                case 100: szModel = g_szMoneyModel;
                case 200: szModel = g_szHealthModel;
                case 400: szModel = g_szXPModel;
                case 500: szModel = g_szYangModel;
                default:  szModel = g_szHealthModel;
            }
            engfunc(EngFunc_SetModel, iEnt, szModel);
            engfunc(EngFunc_SetOrigin, iEnt, fOrigin);

            switch (iType) {
                case 100: util_set_rendering(iEnt, kRenderFxGlowShell, 0, 255, 0, 20);   // verde - bani
                case 200: util_set_rendering(iEnt, kRenderFxGlowShell, 255, 0, 0, 20);   // rosu - HP
                case 400: util_set_rendering(iEnt, kRenderFxGlowShell, 0, 150, 255, 20); // albastru - XP
                case 500: util_set_rendering(iEnt, kRenderFxGlowShell, 255, 215, 0, 20); // auriu - Yang
            }
        }
    }
}
