/*================================================================================
	...
	Versiune 1.0.3 - pure ASCII (fara diacritice) - compileaza curat
================================================================================*/
...
#define PLUGIN  "Metin2 RPG Core"
#define VERSION "b0.0.1"
...
new Float:g_GlobalCooldown[MAX_PLAYERS + 1];

new cvar_xp_kill, cvar_xp_hs, cvar_yang_kill, cvar_yang_hs;
new cvar_upgrade_destroy, cvar_global_cd;
...
public plugin_init()
{
	...
	cvar_global_cd       = register_cvar("amx_metin2_global_cooldown", "10.0");
...
public OnPlayerSpawn(id)
{
	...
	for (new i = 0; i < MAX_EQUIP_SLOTS; i++)
	{
		new itemid = g_Player[id][g_Equipped][i];
		if (itemid > 0 && itemid < TOTAL_ITEMS)
			speed += float(g_ItemData[itemid][7] + g_Player[id][g_EquippedUpgrade][i]);
	}
...
public OnTakeDamage(victim, inflictor, attacker, Float:damage, damagebits)
{
	...
	if (weap_id > 0 && weap_id < TOTAL_ITEMS)
		weap_bonus = float(g_ItemData[weap_id][2] * 2 + weap_upg * 3);
	...
	if (armor_id > 0 && armor_id < TOTAL_ITEMS)
		def += float(g_ItemData[armor_id][3] + g_Player[victim][g_EquippedUpgrade][SLOT_ARMOR] * 2);
...
public Task_HUD()
{
	for (new id = 1; id <= MaxClients; id++)
	{
		...
		set_hudmessage(0, 255, 100, -1.0, 0.92, 0, 0.0, 1.1, 0.0, 0.0, -1);
		ShowSyncHudMsg(id, g_HudSync, "[Metin2 CS 1.6] Rasa: %s | Lvl: %d | XP: %d/%d | Yang: %d | Stat: %d | Skill P: %d",
...
public race_handler(id, menu, item)
{
	...
	client_print_color(id, print_team_default, "^4[Metin2]^1 Ai ales rasa: ^3%s^1! Foloseste ^3/stats^1 si ^3/skills^1.", g_RaceName[race]);
	save_player(id);
...
public cmd_stats(id)
{
	...
	new title[64];
	formatex(title, charsmax(title), "\yStatut - Puncte disponibile: %d", g_Player[id][g_StatPoints]);
...
stock bool:can_use_skill(id, skill_slot)
{
	...
	if (now < g_GlobalCooldown[id])
	{
		client_print_color(id, print_team_default, "^4[Metin2]^1 Cooldown global activ!");
		return false;
	}
...
stock apply_skill_cooldown(id, skill_slot, Float:base_cd)
{
	...
	new Float:cd = base_cd - (float(lvl) * 0.15);
	if (cd < 3.0)
		cd = 3.0;
	
	g_SkillCooldown[id][skill_slot] = get_gametime() + cd;
	g_GlobalCooldown[id] = get_gametime() + get_pcvar_float(cvar_global_cd);
}
...
stock execute_skill(id, slot)
{
	...
	client_print_color(id, print_team_default, "^4[Metin2]^1 Activezi: ^3%s", g_SkillName[skill_idx]);
...
stock skill_warrior(id, slot, lvl)
{
	switch (slot)
	{
		case 0:
		{
			new Float:dur = 8.0 + (lvl * 0.2);
			set_user_rendering(id, kRenderFxGlowShell, 255, 50, 0, kRenderNormal, 35);
			set_task(dur, "RemoveGlow", id);
			apply_skill_cooldown(id, 0, 25.0);
			client_print_color(id, print_team_default, "^4[Metin2]^1 Aura Sabiei activata pentru %.1fs!", dur);
		}
		case 1:
		{
			new Float:dur = 6.0 + (lvl * 0.15);
			set_user_rendering(id, kRenderFxGlowShell, 0, 100, 255, kRenderNormal, 30);
			set_task(dur, "RemoveGlow", id);
		...
		case 2:
		{
			...
			if (is_user_alive(target))
			{
				...
				set_task(1.5 + (lvl * 0.05), "Unfreeze", target);
				apply_skill_cooldown(id, 2, 20.0);
			}
		}
		case 3:
		{
            new Float:origin[3];
            new Float:angles[3];
            new Float:ifforward[3];

            pev(id, pev_origin, origin);
            pev(id, pev_v_angle, angles);
            angle_vector(angles, ANGLEVECTOR_FORWARD, ifforward);

            origin[0] += ifforward[0] * 250.0;
            origin[1] += ifforward[1] * 250.0;
		...
		case 4:
		{
			...
			for (new i = 1; i <= MaxClients; i++)
			{
				...
				if (get_distance_f(origin, torigin) < 200.0)
				{
					new Float:dmg = 40.0 + (lvl * 3.0);
...
stock skill_sura(id, slot, lvl)
{
	switch (slot)
	{
		case 0:
		{
			...
			client_print_color(id, print_team_default, "^4[Metin2]^1 Tais Vrajit activ!");
		}
		case 1:
		{
			apply_skill_cooldown(id, 1, 35.0);
			client_print_color(id, print_team_default, "^4[Metin2]^1 Armura Vrajita - reflect activ!");
		}
		case 2:
		{
		...
		case 3:
		{
			...
			if (is_user_alive(target))
			{
				set_user_maxspeed(target, 120.0);
				set_task(3.0 + (lvl * 0.1), "RestoreSpeed", target);
				apply_skill_cooldown(id, 3, 20.0);
			}
		}
		case 4:
		{
			apply_skill_cooldown(id, 4, 40.0);
		}
	}
}
...
stock skill_ninja(id, slot, lvl)
{
	switch (slot)
	{
		case 0:
		{
			new Float:dur = 5.0 + (lvl * 0.15);
			set_user_rendering(id, kRenderFxNone, 0, 0, 0, kRenderTransAlpha, 10);
			set_task(dur, "RemoveInvis", id);
			apply_skill_cooldown(id, 0, 30.0);
		}
		case 1:
		{
			set_user_maxspeed(id, 450.0 + (lvl * 5.0));
			set_task(6.0, "RestoreSpeed", id);
			apply_skill_cooldown(id, 1, 25.0);
		}
		case 2:
		{
			apply_skill_cooldown(id, 2, 20.0);
		...
		case 3:
		{
			...
			if (is_user_alive(target))
			{
				...
				set_task(1.0, "PoisonTick", taskid, _, _, "a", 5 + (lvl / 5));
				apply_skill_cooldown(id, 3, 22.0);
			}
		}
		case 4:
		{
			apply_skill_cooldown(id, 4, 35.0);
		}
	}
}
...
stock skill_shaman(id, slot, lvl)
{
	switch (slot)
	{
		case 0:
		{
			...
			client_print_color(id, print_team_default, "^4[Metin2]^1 Ai vindecat ^3%d HP", heal);
		}
		case 1:
		{
			apply_skill_cooldown(id, 1, 40.0);
		}
		case 2:
		{
			apply_skill_cooldown(id, 2, 30.0);
		}
		case 3:
		{
			for (new i = 0; i < 5; i++)
				g_SkillCooldown[id][i] = 0.0;
			apply_skill_cooldown(id, 3, 60.0);
			client_print_color(id, print_team_default, "^4[Metin2]^1 Toate cooldown-urile resetate!");
		}
		case 4:
		{
			new Float:origin[3];
			...
			message_begin(MSG_PVS, get_user_msgid("ScreenFade"), origin);
			write_short(1<<10);
			write_short(1<<10);
			write_short(1<<12);
			write_byte(255);
...
public PoisonTick(taskid)
{
	...
	if (is_user_alive(target) && is_user_connected(attacker))
		ExecuteHamB(Ham_TakeDamage, target, attacker, attacker, 8.0, DMG_POISON);
}
...
public unequip_handler(id, menu, item)
{
	...
	g_Player[id][g_Inventory][idx] = g_Player[id][g_Equipped][slot];
...
public shop_handler(id, menu, item)
{
	...
	client_print_color(id, print_team_default, "^4[Metin2]^1 Ai cumparat ^3%s +0", g_ItemName[itemid]);
	save_player(id);
...
public cmd_binds(id)
{
	client_print_color(id, print_team_default, "^4[Metin2]^1 === Ghid Bind-uri Skill ===");
	client_print_color(id, print_team_default, "^3bind f skill1");
	client_print_color(id, print_team_default, "^3bind g skill2");
	client_print_color(id, print_team_default, "^3bind h skill3");
	client_print_color(id, print_team_default, "^3bind j skill4");
	client_print_color(id, print_team_default, "^3bind k skill5");
	client_print_color(id, print_team_default, "^1Tastele pot fi schimbate dupa preferinta.");
	return PLUGIN_HANDLED;
}
