/*================================================================================
	...
	Versiune 1.1 - Skill-uri reale + Mana + Item bonuses + Potions
================================================================================*/
...
#define VERSION "b0.2" //Not a stable working verison yet!
...
public Task_HUD()
{
	for (new id = 1; id <= MaxClients; id++)
	{
		...
		ShowSyncHudMsg(id, g_HudSync, "[Metin2] %s | Lvl %d | XP %d/%d | Yang %d | MP %d/%d | Stat %d | SkillP %d",
...
stock skill_warrior(id, slot, lvl)
{
	...
	switch (slot)
	{
		case 0: // Aura Sabiei - damage boost
		{
			...
			client_print_color(id, print_team_default, "^4[Metin2]^1 Aura Sabiei activa %.1f secunde!", dur);
		}
		case 1: // Corp Rezistent
		{
			...
			// Reducere damage se face prin flag (simplificat - aici doar vizual + mesaj)
			apply_skill_cooldown(id, 1, 28.0);
			client_print_color(id, print_team_default, "^4[Metin2]^1 Corp Rezistent activ!");
		}
		case 2: // Izbitura - stun
		{
			...
			if (is_user_alive(target))
			{
				...
				client_print_color(id, print_team_default, "^4[Metin2]^1 Inamic inghetat %.1fs!", stun);
			}
			else apply_skill_cooldown(id, 2, 18.0);
		}
		case 3: // Dash
		{
			new Float:origin[3];
...
{
	new Float:power = float(lvl) * 1.6 + float(g_Player[id][g_INT]) * 0.8;
	
	switch (slot)
	{
		case 0: // Tais Vrajit - damage + lifesteal (simplificat)
		{
			g_AuraActive[id] = true; // reutilizam flag-ul pentru damage boost
			new Float:dur = 7.0 + (lvl * 0.2);
			set_task(dur, "RemoveAura", id);
		...
		case 1: // Reflect
		{
			...
			client_print_color(id, print_team_default, "^4[Metin2]^1 Armura Vrajita - reflect activ %.1fs!", dur);
		}
		case 2: // Armor Pierce (urmatoarele lovituri)
		{
			apply_skill_cooldown(id, 2, 16.0);
			client_print_color(id, print_team_default, "^4[Metin2]^1 Armor Pierce activ!");
		}
		case 3: // Slow
		{
			new target = get_aim_target(id);
			if (is_user_alive(target))
			{
				...
				set_task(3.5 + (lvl * 0.1), "RestoreSpeed", target);
				apply_skill_cooldown(id, 3, 18.0);
			}
			else apply_skill_cooldown(id, 3, 18.0);
		}
		case 4: // Pietrificare / Miss chance (simplificat - mesaj)
		{
			apply_skill_cooldown(id, 4, 38.0);
			client_print_color(id, print_team_default, "^4[Metin2]^1 Pietrificare activata!");
		}
	}
}
...
stock skill_ninja(id, slot, lvl)
{
	switch (slot)
	{
		...
		case 1: // Speed
		{
			...
			set_task(6.0 + (lvl * 0.1), "RestoreSpeed", id);
		...
		case 2: // Ambush
		{
			...
			apply_skill_cooldown(id, 2, 18.0);
			client_print_color(id, print_team_default, "^4[Metin2]^1 Ambush pregatit! Urmatorul atac e critic.");
		}
		case 3: // Poison
		{
			new target = get_aim_target(id);
			...
			else apply_skill_cooldown(id, 3, 20.0);
		}
...
stock skill_shaman(id, slot, lvl)
{
	...
	switch (slot)
	{
		...
		case 1: // Atac Intens (team buff - simplificat self)
		{
			g_AuraActive[id] = true;
			set_task(8.0 + (lvl * 0.15), "RemoveAura", id);
			apply_skill_cooldown(id, 1, 35.0);
			client_print_color(id, print_team_default, "^4[Metin2]^1 Atac Intens activ!");
		}
		case 2: // Binecuvantare
		{
			apply_skill_cooldown(id, 2, 28.0);
			client_print_color(id, print_team_default, "^4[Metin2]^1 Binecuvantare activa!");
		}
		case 3: // Iutesenie - reset cooldowns
		{
			...
			client_print_color(id, print_team_default, "^4[Metin2]^1 Toate cooldown-urile au fost resetate!");
		}
		case 4: // Flash
		{
			new Float:origin[3];
...
// ======================== MENIURI (aceleași ca înainte + potiuni) ========================
...
// Inventar, Upgrade, Shop, Binds - pastreaza logica anterioara + potiuni
